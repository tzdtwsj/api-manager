...
$ms = new MineStat($get_ip, $get_port, 1, $get_type);
/*
...
if ($ms->is_online()){
	...
	$ip = $ms->get_address();
	$port = $ms->get_port();
	$connect_type = $ms->get_request_type();
	$protocol = $ms->get_protocol();
	$current_players = $ms->get_current_players();
	$maxplayers = $ms->get_max_players();
	$version = $ms->get_version();
	$motd = $ms->get_stripped_motd();
	if ($ms->get_request_type() == "Bedrock/Pocket Edition"){
		$gamemode = $ms->get_mode();
	} else {
		$favicon = $ms->get_favicon_b64();
	}
	$ping = $ms->get_latency();
} else {
	$status = "failed";
...
echo json_encode(["status"=>$status,"ip"=>$ip,"port"=>$port,"connect_type"=>$connect_type,"data"=>["protocol"=>$protocol,"current_players"=>$current_players,"maxplayers"=>$maxplayers,"version"=>$version,"motd"=>$motd,"gamemode"=>$gamemode,"ping"=>$ping."ms","favicon"=>$favicon]],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);
?>